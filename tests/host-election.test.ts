 ...
 *     code, each convinced they are the host, seeing nobody. In Delvepack the host
...
async function peer(id: string, opts: { claimHost?: boolean } = {}): Promise<Net> {
  ...
  return mod.createNet({ appId: 'delvepack', roomId: 'R', claimHost: opts.claimHost });
}
...
describe('host election — nobody hosts a mesh that has not formed', () => {
  ...
  it('falls back to an election if the room turns out to have no host', async () => {
    ...
    //
    // The settle window is 6s in engine v1.1.0, up from 2.5s. That is the whole
    // point: Nostr discovery plus ICE on a mobile network routinely takes longer
    // than 2.5s, and a joiner that gave up early self-elected and then stole a
    // live room. Waiting past the real window is what makes this a fallback
    // rather than a race.
